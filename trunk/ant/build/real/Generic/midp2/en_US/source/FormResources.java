...
public class FormResources extends Form implements CommandListener, ChildForm, Updatable {
    ...
    public FormResources(String s, ChildForm parent) {
        super(s);
            ...
            this.parent = parent;
            //#style mainScreenF
            this.menuScreen = new List("Resources", List.IMPLICIT, StyleSheet.mainscreenfStyle );
            Enumeration en = JCageConfigurator.resources.keys();
            
            while (en.hasMoreElements()) {
                Resource res = (Resource) JCageConfigurator.resources.get(en.nextElement());
                    //#style mainCommandF
                    menuScreen.append(res.getId() + " " + res.getAmount() /*+ "| +" + res.getRate()*/, MainMid.getImage("/notes.png"), StyleSheet.maincommandfStyle );
    ...
    public void update() {
        ...
        while (en.hasMoreElements()) {
            Resource res = (Resource) JCageConfigurator.resources.get(en.nextElement());
                //#style mainCommandF
                menuScreen.set(i, res.getId() + " " + res.getAmount() /*+ "| +" + res.getRate()*/, MainMid.getImage("/notes.png"), StyleSheet.maincommandfStyle );
